...
The Terminal Logs are Located  on the Appendix on figures \ref
...
The reference lidar point cloud will have an scale of $1$ as it is the real scale of the person. 
...
This process uses PCA analysis to find the Scale between the Lidar point cloud and the recontructed processed point cloud used in the developed framework. 
...
The target point cloud will tranform its pose to have the same transformation as the reference point cloud. This process will also align the corresponding centroids in order to have the correct centroid oientation and align the two point clouds correctly,
...
Based on this verification step it will be possible to detrmine if the scaling and refrence frame matches the reference point cloud.
...
The Scaling Process  is one of the most important processes in the entire reconstruction pipeline. 
This steps allows to estimate the scale and adjust the resulting point cloud.
As mentioned on section \ref{sectoin:Scale_est}, the scale of the lidar will be of $1$, as it is the real scale of the scanned person. 
...
This allow to have the specific regions of interest of only the scanned person in both target and reference point cloud in order to have accurace results.

The results obtained in the scaling process are based on the data set obtain on figure \ref{fig:scan3}.
These will involve the scaling process as well as the verificatation of the final transform between the target and reference point cloud. 

Intially  the two set of point cloud will look similar to figure \ref{fig:scale1}. 
...
\begin{figure}[H]%this figure will be at the right
  \centering
%  \includegraphics[width=1\textwidth]{Step2ForScale01.png}
  \subfloat[Scale Initial Step Comparison 1]{{\includegraphics[width=0.46\textwidth]{Step2ForScale01.png} \label{fig:scale1a}}}
  \qquad
  \subfloat[Scale Initial Step Comparison 2]{{\includegraphics[width=0.46\textwidth]{Step3ForScale02.png} \label{fig:scale1b}}}
  \qquad
...
After the PCA analysis and Transsformation the estimated scale was \textbf{0.9171}. 
Which matches the figure \ref{fig:scale1} as the target poitcloud is \textbf{0.9171} smallar than the reference point cloud.
...
\end{wrapfigure}
With that scale value the target point cloud get adjusted as mentioned on section \ref{sectoin:Scale_est}, in order to match the correct scale of the scanned person. 
...
As mentioned on section \ref{scale_API}, the transform adjustment process will use PCA to obtain the pose of the reference point cloud. Then the target point cloud will be trtansfomed to that reference pose.
...
These obtained results ensure that the scaling  and translation process were sucessful as the target point cloud has the same scale as the reference Lidar point cloud as well as the correct final pose.
...
\begin{figure}[H]%this figure will be at the right
  \centering
%  \includegraphics[width=1\textwidth]{Step2ForScale01.png}
  \subfloat[]{{\includegraphics[width=0.46\textwidth]{Step4ForScale03.png}}}
  \qquad
  \subfloat[]{{\includegraphics[width=0.46\textwidth]{Step5ForScale04.png} }}
  \qquad
  \subfloat[]{{\includegraphics[width=0.46\textwidth]{Step6ForScale05.png} }}
  \qquad
  \subfloat[]{{\includegraphics[width=0.46\textwidth]{Step7ForScale06.png} }}
  \qquad
\caption{Scaling and transformation Results}.
\label{fig:scale_results} 
\end{figure}

On figure \ref{fig:scale_results}, the target scaled point cloud is illustrated with \textit{yellow}, whereas the referece lidar point cloud is on \textit{red}.

Full Resolution images are located in the Appendix. 
...
\begin{table}[H]
  \centering
  \makebox[\textwidth]
  {\includegraphics[width=1\linewidth]{coverletter.png}    
  }
  \caption{Statement of Originality}
  \label{appendix:cover}
  \end{table}
...
\begin{table}[H]
  \centering
  \makebox[\textwidth]
  {\includegraphics[width=0.92\linewidth]{commsLog.png}    
  }
