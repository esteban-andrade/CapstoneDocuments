...
\begin{figure}[H]
  \centering
  \subfloat[RIG From View]{{\includegraphics[width=0.85\textwidth]{IMG_5883.JPG} }}
  \qquad
  \subfloat[RIG lateral View]{{\includegraphics[width=0.85\textwidth]{IMG_5884.JPG} }}
...
This developed framework was develop by \Citet*{scaleICP} and it is a method to scale 3D point clouds. 
...
\chapter{Conclusion}
This project aimmed at the developement of a 3D scanner device that will produce accurate results. 
This report focused on the data processing and resonctruction of the acquired data from the scanned in order to generate a a virtual scaled scanned representation of the person.

As previously mentioned for this project the RIG will capture a set of images that will be used in a photogrammetric process using Meshroom in order to have representation model of the scanned person. 
The generated result will be processed in a reconstruction framework pipeline that will run several algorithms in order to improve the scan. 
Finally the pipeline will scale the model accordinluy and run the Possion Surface Reconstruction in order to obtain the final Reconstructed model of the scanned person. 
The result will be subsequently usable for different applications such as clothing fitting. 

Future developement of this technology will allow to have much more accurate scans andx make this concept commercially viable for different industires such as online retailers, clothing manufacturing,etc. 


\chapter{Recommendations}
Based on the results obtained in this report and future developement and improvements for this technology the following recommendations are proposed.

\begin{itemize}
  \item Create a Morphing suit with distintive featires and patterns that the user of the scan will use, which will allow to have consistent features in every scan.
  \item Add more Cameras in order to have more viewpoints and improve the SFM process.
  \item Add better quality  and resolution cametas in order to have more features in each image.
\end{itemize}
